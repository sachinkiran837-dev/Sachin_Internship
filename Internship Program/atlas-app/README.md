...
- **The columns an org map needs may not exist in the file.** A name split across `FirstName` and `Surname` is joined into one column, or every position carries only a first name and nothing can be matched to it. An hourly rate is annualised into a cost using hours from `config/ingest-assumptions.json` — stated wherever the figure appears, because it is an assumption and not something the file says. Composed columns are *added*, never substituted: the originals stay visible on the confirm screen.
