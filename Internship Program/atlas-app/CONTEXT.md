...
**Coverage as of this pass (Phase 7 — Narrative and outputs, the final phase):** every module of the 27-skill spec (A1–A3, B1–B5, C1–C4, D1–D3, E1–E3, F1, G1–G2, H1–H3, I1–I3) was built out — see the module map below. I1 (board pack) and I2 (consultant briefing) were subsequently removed; they packaged figures the other modules already surface on Findings and computed nothing unique to them. "Built out" does not mean every worked example in every skill doc is implemented to the letter; see the closing "What this build genuinely is, and isn't" section for the honest scope cuts, which apply across every module, not just this last pass.
...
- **I1/I2 removed** — the board pack (one reconciled headline + judgment sentence, `reconcileValue`'s `netStack` verbatim) and consultant briefing (hypotheses ranked `sizing × confidence weight`, high=1/medium=0.6/low=0.3, with a per-archetype anticipated pushback) were both built and verified, then removed as pure repackaging of figures already on Findings. `lib/report/boardPack.ts`, `lib/hypothesis/briefing.ts` and `lib/hypothesis/pushback.ts` no longer exist.
...
| ~~Board pack (I1)~~ / ~~Consultant briefing (I2)~~ | removed — see "What this build genuinely is, and isn't" |
...
| AI wrapper (all AI-shaped behaviours) | `lib/ai/client.ts` |
| Database | `db/schema.ts` (Drizzle), `db/client.ts`, `db/repo.ts` |
| Pages | `app/org/[orgId]/{page,map,scenarios,findings,ask}` |
...
F1's own dedicated output — the "How you compare" section — surfaces as ordinary Findings-page hypotheses (lens `"External reference"`) the same way D1–D3/E1–E3 do. It briefly had a second home in the board pack's own "How you compare" section; that page has since been removed, so Findings is F1's only surface again.
...
`lib/scenario/reconcile.ts`'s `reconcileValue` reads `analyseAllPlays`'s own output — same as the Scenarios tab does for its per-play cards — and produces the reconciled, no-double-count headline stack across every priced play at once. Not surfaced on the Scenarios tab itself; its one page consumer, the board pack, has been removed, so the reconciled stack currently has no page reading it.
...
- **I1 (board pack) and I2 (consultant briefing) were removed** — both were built and verified against real data, but neither computed anything the other modules don't already surface on Findings; they only packaged G2's reconciled stack, F1's bands, E1's protection story and H3's phase ladder for a board or consultant audience. `lib/report/boardPack.ts`, `lib/hypothesis/briefing.ts`, `lib/hypothesis/pushback.ts` and their pages/verify scripts are gone.
